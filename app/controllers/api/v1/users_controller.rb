class Api::V1::UsersController < ApplicationController
 ...
 def index
     users = User.all
     render json: users, except: [:updated_at, :created_at], include:[:notes, :medications, :disease_states]
 end
 

 def show
    user = User.find(params[:id])
    render json: user, except: [:updated_at, :created_at], include:[:notes, :medications, :disease_states]
end
